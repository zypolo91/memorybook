 ...
 * 相册记忆关联表
 */
export const albumMemories = pgTable('album_memories', {
  id: serial('id').primaryKey(),
...
export const familyMembers = pgTable('family_members', {
  id: serial('id').primaryKey(),
  circleId: integer('circle_id').notNull().references(() => familyCircles.id, { onDelete: 'cascade' }),
  userId: integer('user_id').notNull().references(() => users.id),
 ...
 * 收藏表
 */
export const memoryFavorites = pgTable('memory_favorites', {
  id: serial('id').primaryKey(),
  memoryId: integer('memory_id').notNull().references(() => memories.id, { onDelete: 'cascade' }),
  userId: integer('user_id').notNull().references(() => users.id),
  createdAt: timestamp('created_at').defaultNow(),
...
export const memoryReminders = pgTable('memory_reminders', {
  ...
  reminderTimeIdx: index('memory_reminders_reminder_time_idx').on(table.reminderTime),
}));
...
export const familyCirclesRelations = relations(familyCircles, ({ one, many }) => ({
  creator: one(users, {
...
export const memoryFavoritesRelations = relations(memoryFavorites, ({ one }) => ({
  memory: one(memories, {
...
export const memoryRemindersRelations = relations(memoryReminders, ({ one }) => ({
  user: one(users, {
